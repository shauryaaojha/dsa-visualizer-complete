  ...
  const stateInspector = useMemo(() => {
    ...
    return {
      variables: {
        ...
        height: tree ? Math.floor(Math.log2(countNodes(tree))) + 1 : 0,
        traversalHistory: traversalHistory.length,
      },
      totalSteps: steps.length || 1,
      currentStepDescription: `Tree with ${countNodes(tree)} node${countNodes(tree) !== 1 ? "s" : ""}`,
    }
  }, [tree, traversalHistory, steps.length])

  return (
    <div className="container mx-auto space-y-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight">Binary Search Tree</h1>
        <p className="text-muted-foreground">
          A binary tree that maintains sorted data with O(log n) operations.
        </p>
      </div>
